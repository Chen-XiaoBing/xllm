...
namespace xllm {
using OxygenVLMImageInputs = Qwen2_5_VLImageInputs;

struct OxygenVLMVideoInputs {
...
//         "position_embedding", torch::nn::Embedding(num_positions_,
//         embed_dim_));
...
//       const int64_t orig_size =
//       static_cast<int64_t>(std::sqrt(orig_size_sq)); auto pos_embed_2d =
...
//       auto grid = torch::stack({norm_w, norm_h},
//       -1).unsqueeze(0).unsqueeze(2); namespace F = torch::nn::functional;
...
//                                                    .align_corners(false));
//       adapted_pos_embed =
//           interpolated_embed.squeeze(0).squeeze(-1).permute({1,
//           0}).to(dtype);
//     }
//
...
//     Glm4VisionRotaryEmbedding(context)); rotary_pos_emb_ =
//     register_module("rotary_pos_emb",
...
//   }
//   std::tuple<torch::Tensor, torch::Tensor> rot_pos_emb(torch::Tensor
//   grid_thw) {
...
//     rotary_pos_emb_(max_grid_size.item<int64_t>()); auto rotary_pos_emb =
//     rotary_pos_emb_full.index({pos_ids}).flatten(1);
//
...
//     cu_seqlens = F::pad(
//         cu_seqlens, F::PadFuncOptions({1,
//         0}).mode(torch::kConstant).value(0));
...
//         {-1, spatial_merge_size_, spatial_merge_size_,
...
class OxygenVLMVisionPatchEmbedImpl : public torch::nn::Module {
 public:
  OxygenVLMVisionPatchEmbedImpl(const ModelContext& context) {
    auto model_args = context.get_model_args();
    auto options = context.get_tensor_options();
...
TORCH_MODULE(OxygenVLMVisionPatchEmbed);

// class OxygenVLMVisionRmsNormImpl: public torch::nn::Module {
//  public:
//   torch::Tensor weight;
//   OxygenVLMVisionRmsNormImpl(const ModelContext& context) {
//     auto model_args = context.get_model_args();
//     auto options = context.get_tensor_options();
//     weight = torch::empty({model_args.mm_hidden_size()}, options);
//     epsilon_ = 1e-5;
...
//   double epsilon_;
// };
// TORCH_MODULE(OxygenVLMVisionRmsNorm);

class OxygenVLMVisionEmbeddingsImpl : public torch::nn::Module {
 public:
  OxygenVLMVisionEmbeddingsImpl(const ModelContext& context) {
    auto model_args = context.get_model_args();
    auto options = context.get_tensor_options();
    embed_dim_ = model_args.mm_hidden_size();
    image_size_ = model_args.mm_image_size();
...
TORCH_MODULE(OxygenVLMVisionEmbeddings);

class OxygenVLMVisionPatchMergerImpl : public torch::nn::Module {
 public:
  OxygenVLMVisionPatchMergerImpl(const ModelContext& context) {
    auto model_args = context.get_model_args();
    options_ = context.get_tensor_options();
...
class OxygenVLMVisionTransformerImpl : public torch::nn::Module {
 public:
  OxygenVLMVisionTransformerImpl(const ModelContext& context)
      : options_(context.get_tensor_options()) {
    auto model_args = context.get_model_args();
    spatial_merge_size_ = model_args.mm_spatial_merge_size();
    hidden_size_ = model_args.mm_hidden_size();
    ...
    patch_embed_ =
        register_module("patch_embed", OxygenVLMVisionPatchEmbed(context));
    rotary_pos_emb_ = register_module("rotary_pos_emb",
                                      Qwen2_5_VisionRotaryEmbedding(context));
    // post_conv_layernorm_ =
    //     register_module("post_conv_layernorm",
    //     OxygenVLMVisionRmsNorm(context));
    ...
    embeddings_ =
        register_module("embeddings", OxygenVLMVisionEmbeddings(context));

    blocks_ = register_module("blocks", torch::nn::ModuleList());

    for (int32_t idx = 0; idx < model_args.mm_num_hidden_layers(); idx++) {
      auto block = OxygenVLMVisionBlock(context);
      blocks_->push_back(block);
      layers_.push_back(block);
    }
    // post_layernorm_ =
    //     register_module("post_layernorm", OxygenVLMVisionRmsNorm(context));
    ...
    downsample_->weight.set_data(downsample_->weight.to(options_));
    downsample_->bias.set_data(downsample_->bias.to(options_));
    merger_ = register_module("merger", OxygenVLMVisionPatchMerger(context));
  }
 ...
 private:
  ...
  OxygenVLMVisionPatchEmbed patch_embed_{nullptr};
  Qwen2_5_VisionRotaryEmbedding rotary_pos_emb_{nullptr};
  torch::nn::ModuleList blocks_{nullptr};
  OxygenVLMVisionEmbeddings embeddings_{nullptr};
  layer::RMSNorm post_conv_layernorm_{nullptr};
  layer::RMSNorm post_layernorm_{nullptr};
  torch::nn::Conv2d downsample_{nullptr};
  std::vector<OxygenVLMVisionBlock> layers_;
  OxygenVLMVisionPatchMerger merger_{nullptr};
  torch::TensorOptions options_;
  bool is_post_conv_layernorm_weight_loaded = false;
  bool is_post_layernorm_weight_loaded = false;
  bool is_downsample_weight_loaded_ = false;
...
TORCH_MODULE(OxygenVLMVisionTransformer);
...
class OxygenvlmForConditionalGenerationImpl : public torch::nn::Module {
 public:
  OxygenvlmForConditionalGenerationImpl(const ModelContext& context)
      : model_args_(context.get_model_args()),
        options_(context.get_tensor_options()) {
    visual_ = register_module("visual", OxygenVLMVisionTransformer(context));
  ...
  void prepare_encoder_input(
      const ModelInputParams& input_params,
      std::optional<OxygenVLMImageInputs>& image_inputs,
      std::optional<OxygenVLMVideoInputs>& video_inputs) {
    ...
    if (pixel_values.defined() && image_grid_thw.defined())
      image_inputs = OxygenVLMImageInputs{pixel_values, image_grid_thw};

    if (pixel_values_videos.defined() && video_grid_thw.defined())
      video_inputs = OxygenVLMVideoInputs{pixel_values_videos, video_grid_thw};
  }
  ...
  MMDict get_multimodal_embeddings(const ModelInputParams& input_params) {
    ...
    std::optional<OxygenVLMImageInputs> image_input;
    std::optional<OxygenVLMVideoInputs> video_input;
    ...
    if (image_input) {
      ...
      LOG(INFO) << "image_embeds: " << get_shape_str(image_embeds)
                << ", image_tokens_vec: " << image_tokens_vec;
      multimodal_embeds["image|embedding"] =
          image_embeds.split(image_tokens_vec, 0);
      LOG(INFO) << "finish visual";
    }
    if (video_input) {
  ...
  ModelOutput forward(const torch::Tensor& tokens,
                      ...
                      const ModelInputParams& input_params) {
    LOG(INFO) << "start language_model forward";
    auto emb = language_model_(tokens, positions, kv_caches, input_params);
    LOG(INFO) << "finish language_model forward";
    return emb;
  }
 ...
 private:
  ...
  torch::TensorOptions options_;
  OxygenVLMVisionTransformer visual_{nullptr};
