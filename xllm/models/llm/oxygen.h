...
class OxygenModelImpl : public QWen3ModelImpl {
 public:
  ...
  virtual ModelOutput forward(torch::Tensor tokens,
    ...
    LOG(INFO) << "Start OxygenModel forward";
    ...
    torch::Tensor h;
    LOG(INFO) << "Start OxygenModel input embed";
    if (inputs_embeds.defined()) {
      h = inputs_embeds;
    } else {
      h = embed_tokens_(tokens);
    }
    LOG(INFO) << "input embed defined: " << inputs_embeds.defined();
    LOG(INFO) << "OxygenForCausalLM input embed: " << h;
    LOG(INFO) << "Finish OxygenModel input embed";
    if (use_deepstack) {
    ...
    LOG(INFO) << "Start OxygenModel apply mrope";
    ...
    LOG(INFO) << "Finish OxygenModel apply mrope";
